...
inputs_path = Path('./train/inputs.npy')
outputs_path = Path('./train/outputs.npy')
test_inputs_path = Path('./test/test_inputs.npy')
...
class BertForHouseQA(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(os.path.join(PRETRAIN_MODEL_DIR, 'bert-base-chinese'), 
...
def train_pytorch(**kwargs):
    ...
    for fold, (train_idx, valid_idx) in enumerate(gkf):
        ...
        for epoch in range(kwargs['epoch']):
            ...
            valid_f1, _ = search_f1(valid_outputs, valid_pred)
            ...
            }, f=os.path.join(CHECKPOINT_DIR, 'ckpt.pt'))

train_pytorch(batch_size=256, epoch=3, inputs=inputs, outputs=outputs, test_inputs=test_inputs)
