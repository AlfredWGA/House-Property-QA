...
class Batcher():
    def __init__(self, dataset_name, tokenizer, args):
        ...
        if not dataset_name == 'dev':
            ...
            query_list = list(self.query.keys())
            train_set, test_set = train_test_split(query_list, test_size=0.2, random_state=42)
            if dataset_name == 'train':
                for qid, did, label in qd_pairs:
                    if qid in train_set:
                        self.qd_pairs_id.append((qid, did, label))
                ''' 对数据进行 shuffle '''
                if self.args.shuffle == 'True':
                    random.shuffle(self.qd_pairs_id)
            else:
                for qid, did, label in qd_pairs:
                    if qid in test_set:
                        self.qd_pairs_id.append((qid, did, label))
